// Shared tool registration for both transports: local stdio (index.ts)
// and remote Streamable HTTP on Vercel (api/mcp.ts).
...
export function buildServer(): McpServer {
  ...
  const server = new McpServer({ name: "commonplace", version: "0.3.0" });
